...
#
#  Handles:
#   1. Sensor serial (Arduino 1 → Pi)
#      Reads: m/M, p/P, v/V, g/G, O/C, G:lat,lng
#   2. Camera / YOLO detection (thread)
#      Detects items visually, logs to DB
#   3. Motor serial (Pi → Arduino 2)
#      Sends: P, M, V servo commands
#
#  Ports:
#   /dev/ttyACM0 → Arduino 1 (sensors, read only)
#   /dev/ttyACM1 → Arduino 2 (motors, write only)
# ─────────────────────────────────────────────
...
# ── Config ────────────────────────────────────
SENSOR_PORT = '/dev/ttyACM0'   # Arduino 1 — sensors
MOTOR_PORT  = '/dev/ttyACM1'   # Arduino 2 — motors
BAUD_RATE   = 9600
...
YOLO_MODEL_URL        = 'https://hub.ultralytics.com/models/g6ukybMzSi9dDpghhDTY'
...
# Motor serial connection (Pi → Arduino 2)
motor_serial = None
...
# Servo command chars sent to motor Arduino
SERVO_CHAR = {
    'plastic':  'P',
    'metal':    'M',
    'valuable': 'V',
...
def send_servo(category):
    ...
    if char and motor_serial and motor_serial.is_open:
        ...
        log.warning(f"No servo command defined for category: {category}")
    ...
    try:
        while True:
                ...
                if raw is not None:
                    ...
                    for det in raw:
                        ...
                        if category:
                            ...
                            log.info(f"Camera detected: {det['name']} → {category}")
                            log_item(category)
                            send_servo(category)
                        else:
            ...
            for det in current_detections:
                ...
                label    = f"{category.upper()} ({det['name']}) {det['confidence']:.2f}"
...
def main():
    ...
    try:
        ...
        log.warning(f"Motor Arduino not found on {MOTOR_PORT}: {e}")
